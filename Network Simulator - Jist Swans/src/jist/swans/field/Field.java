...
import guiTrace.LinkTraceEvent;
import guiTrace.NodeTraceEvent;
import guiTrace.Trace;
import guiTrace.TraceEvent;
...
public class Field implements FieldInterface
{
  ...
  public void moveRadio(Integer id, Location loc)
  {
	...
	//System.out.println("Move Node["+id+"] to location ["+loc.getX()+", "+loc.getY()+"]");
    
    
    //GUI-SUPPORT for SWANS
	try {
		this.getTrace().handleEvent(new NodeTraceEvent(loc, id));
	} catch (IOException e) {
  ...
  //
  /**
   * Captures the whole simulation field into teh given Trace object.
   * The output includes all nodes and links between nodes.
   * Links are calculated according to 
   * 
   * Possible improvement: to print to Trace from within moveRadio()
   * in order to capture movement
   */
	public void snapShot2Trace() throws IOException{
		
		for (int i=0; i<radios.length; i++){
  	      RadioData radio = radios[i];
  	      if(radio==null){
  	          continue;
  	      }
  	      TraceEvent event = new NodeTraceEvent(radio.loc, radio.info.getUnique().getID());
          this.getTrace().handleEvent(event);
  	    }
		
		
		//Link traces--Ommited because in wireless networks no links are established in nam!
  	    //snapShotLinks2Trace(tr); 
  	}
	
	  /**
	   * GuiTrace visitor interface.
	   */
	  protected static interface GuiTraceVisitor 
	      extends Spatial.SpatialTransmitVisitor, Spatial.SpatialVisitor
	  {
	  }
	  
	  /**
	   * Output the field links into the Trace object.
	   * This code is almost identical to computeAvgConnectivity() and
	   * ConnectivityVisitor. I did not want to change the existing code. 
	   * @param tr the Trace object
	   */
	  protected void snapShotLinks2Trace(final Trace tr)
	  {
		  // create new gui visitor
	      GuiTraceVisitor guiVisitor = new GuiTraceVisitor()
		  {
	        // snapshot only receive and not sense links
	        final boolean sense = false; 
			public double computeSignal(RadioInfo srcInfo, Location srcLoc, Location dstLoc)
			{
	  			return transmitVisitor.computeSignal(srcInfo, srcLoc, dstLoc);
			}
				...
				Message msg, Long durationObj)
			{
	  			...
	  			// output the link
			  	TraceEvent event = new LinkTraceEvent(srcInfo.getUnique().getID(), 
			  	        	dstInfo.getUnique().getID());
		        try {
	    ...
	    spatial.visit(guiVisitor);
	    return;
	  }
	  
	  public double getFieldDimensionX()
	  {
		  return this.spatial.getTopRight().getX();
	  }
	  
	  public double getFieldDimesionY()
	  {
		  return this.spatial.getTopRight().getY();  
	  }
