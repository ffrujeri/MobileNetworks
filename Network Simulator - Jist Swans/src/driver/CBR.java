...
public class CBR
{
  ...
  /** Gui Support for visulation in Javis. */
  private static final boolean guiSupport = true;
  //////////////////////////////////////////////////
  // locals
  //
      ...
      TransInterface.SocketHandler handler = new TransInterface.SocketHandler()
        {
          ...
          {
            System.out.println("Server:"+localAddr.getIP().getHostAddress()+" Received message " + (++packetsReceived) + " from " + src);
          }
        };
  ...
  private static void buildField(CommandLineOptions opts)
  {
    ...
    // initialize field
    Field field = new Field(opts.field, false);
    ...
    {
    	//JAVIS -GUI SUPPORT
    	JavisTrace.createTraceSetTrace(field,"CBR_"+opts.nodes+"NodeSim");
    ...
    for (int i=1; i<=opts.nodes; i++)
    {
      ...
      NetIp net = new NetIp(address, protMap, loss, loss,field.getTrace());
    ...
    //If gui Support enabled, then draw trace.
    if(guiSupport)
    {
    	//JAVIS -GUI SUPPORT
